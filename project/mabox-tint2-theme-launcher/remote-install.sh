...
## $ export DEBUG_MABOX_TINT2_THEME_LAUNCHER_INSTALL=true
is_debug () {
	if [ "$DEBUG_MABOX_TINT2_THEME_LAUNCHER_INSTALL" = "true" ]; then
		return 0
	fi
...
base_var_init () {
	...
	if [ "none${THE_CMD_FILE_NAME}" = 'none' ]; then
		THE_CMD_FILE_NAME="mabox-tint2-theme-launcher-install"
	fi
